...
- strong product story already exists across README, review pack, replay evals, and architecture docs
- deterministic replay proof is already present and reproducible
- service-review endpoints (`/api/healthz`, `/api/meta`, `/api/review-pack`, `/api/live-session-pack`, `/api/schema/report`) are already meaningful
...
1. **README/docs polish**
   - tighten the top-level reviewer path
...
4. **Low-risk code quality cleanup**
   - reduce obvious duplication in reviewer/export helper logic without changing behavior
   - keep or add regression protection where cleanup touches behavior-sensitive logic

## Acceptance criteria
- a reviewer can understand what AegisOps is, why it matters, and how to verify it in under 3 minutes
- proof assets and review endpoints are easier to discover from the README/docs
...
## Planned passes
1. **Docs / framing pass**
   - README navigation and reviewer quick path
...
3. **Code cleanup pass**
   - simplify duplicated clipboard/reviewer helper flow in `App.tsx`
