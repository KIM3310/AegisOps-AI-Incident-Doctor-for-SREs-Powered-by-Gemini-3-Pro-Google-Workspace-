...
export async function geminiAnalyzeIncident(input: {
  ...
  const candidateModels = Array.from(
    new Set([input.model, input.fallbackModel].filter((value): value is string => Boolean(value?.trim())))
  );
  ...
  throw lastError instanceof Error ? lastError : new Error("Unexpected execution path");
}
...
export async function geminiFollowUp(input: {
  ...
  for (const model of candidateModels) {
    try {
      ...
      const text = (await getResponseText(response)).trim();
      return (
        asString(text, "No answer generated.", 8_000) ||
        "No answer generated."
      );
    } catch (error) {
      lastError = error;
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error("Gemini follow-up request failed");
}
