...
export interface ServerConfig {
  ...
  geminiApiKey?: string;
  geminiFallbackModelAnalyze?: string;
...
export function loadConfig(): ServerConfig {
  ...
  const mode: DemoMode =
    llmProvider === "ollama"
      ...
      ? "live"
      : "demo";
  const modelAnalyze = (process.env.GEMINI_MODEL_ANALYZE?.trim() || "gemini-3.1-pro-preview");
  const geminiFallbackModelAnalyze =
    process.env.GEMINI_MODEL_ANALYZE_FALLBACK?.trim() || "gemini-2.5-flash";
  ...
  return {
    ...
    geminiFallbackModelAnalyze:
      geminiFallbackModelAnalyze || undefined,
    modelAnalyze,
    modelTts,
