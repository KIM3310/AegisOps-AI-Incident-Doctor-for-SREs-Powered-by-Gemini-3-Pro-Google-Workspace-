...
import { persistAnalyzeArtifactsToGcp } from "./lib/gcp-persistence";
...
function parseScorecardFocus(value: unknown): RuntimeScorecardFocus | null {
  const normalized = String(value || "").trim().toLowerCase();
  if (!normalized) return "traffic";
  if (
    normalized === "traffic" ||
    normalized === "quality" ||
...
function buildCloudProofPack() {
  const openai = getOpenAiRuntimeContract();
  const aws = getAwsStatus();
  const gcp = getGcpStatus();
  const datadog = getDatadogStatus();

  return {
    ok: true,
    service: "aegisops-cloud-proof",
    version: 1,
    generatedAt: new Date().toISOString(),
    headline:
    ...
    integrations: {
      openai: {
        ...
        deploymentMode: openai.deploymentMode,
        publicLiveApi: openai.publicLiveApi,
        liveModel: openai.liveModel,
        lastLiveRunAt: openai.lastLiveRunAt,
      },
      gcp,
      aws,
      datadog,
    ...
    proofSequence: [
      {
        ...
        surface: "/api/healthz -> /api/runtime/scorecard",
        why: "Confirm provider mode and bounded live posture before trusting any cloud-backed claim.",
      },
      {
      ...
      },
      {
        ...
        surface: "/api/resource-pack -> /api/summary-pack",
        why: "Inspect the report, replay, and handoff surfaces that sit above the cloud adapters.",
      },
    ],
    links: {
      healthz: "/api/healthz",
...
function buildLocalIncidentDataSummary() {
  ...
  const incidentLogPath = path.join(externalDir, "incident_event_log.csv");
  const incidentSummaryPath = path.join(externalDir, "Incident_response.txt");
  return {
    present: existsSync(externalDir),
    files: {
      incidentLogCsv: {
        path: "samples/external/incident_response_log/incident_event_log.csv",
        present: existsSync(incidentLogPath),
        sizeBytes: existsSync(incidentLogPath) ? statSync(incidentLogPath).size : 0,
        rowCount: countCsvRows(incidentLogPath),
        preview: previewCsvRows(incidentLogPath, 2),
      },
      incidentSummaryTxt: {
        path: "samples/external/incident_response_log/Incident_response.txt",
        present: existsSync(incidentSummaryPath),
        sizeBytes: existsSync(incidentSummaryPath) ? statSync(incidentSummaryPath).size : 0,
...
function previewCsvRows(filePath: string, limit: number): Array<Record<string, string>> {
  if (!existsSync(filePath)) {
    return [];
  }
...
async function callGeminiEscalationPreview(options: {
  apiKey: string;
  ...
  const candidateModels = Array.from(
    new Set(
      [options.model, options.fallbackModel, "gemini-2.5-flash-lite"].filter(
  ...
  for (const model of candidateModels) {
    ...
    try {
                ...
                parts: [
                  {
                    text: [
                      ...
                      "Return compact JSON with keys escalationStance, confidenceBand, handoffSummary, evaluationEvidence, commanderMessage, nextAction, source.",
      ...
      if (!response.ok) {
        lastError = new Error(payload?.error?.message || `Gemini request failed (${response.status})`);
        continue;
      }
      ...
      const content = Array.isArray(payload?.candidates?.[0]?.content?.parts)
        ...
        : "";
      if (!content) {
        lastError = new Error("Gemini response content was empty");
        continue;
      }
      ...
      return typeof result === "object" && result !== null
        ? result
        : { source: "gemini", handoffSummary: content };
    } catch (error) {
      lastError =
  ...
  throw lastError instanceof Error ? lastError : new Error("Gemini preview failed");
}

function buildFallbackEscalationPreview(
  bundle: OpenAiIncidentBundle
...
app.get("/api/runtime/scorecard", (req, res) => {
  const focus = parseScorecardFocus(req.query.focus);
  if (!focus) {
    return sendError(
      req,
      res,
      400,
      "focus must be one of traffic, quality, or reliability."
    );
  }
  return res.json(buildRuntimeScorecard(focus));
});

app.get("/api/cloud-proof", (req, res) => {
  return res.json(buildCloudProofPack());
});
...
app.get("/api/resource-pack", (req, res) => {
  return res.json(buildAegisOpsResourcePack());
});
...
app.get("/api/postmortem-pack", (req, res) => {
  return res.json(buildPostmortemPack());
});

app.get("/api/system-design-pack", (req, res) => {
  return res.json(buildSystemDesignPack());
});
...
app.post("/api/live-escalation-preview", async (req, res) => {
  ...
  if (!runtime.publicLiveApi) {
    const effectiveGeminiApiKey = getEffectiveGeminiApiKey();
    if (effectiveGeminiApiKey) {
      try {
        const result = await callGeminiEscalationPreview({
          apiKey: effectiveGeminiApiKey,
          fallbackModel: cfg.geminiFallbackModelAnalyze,
    ...
    return res.json({
      ...
      result: {
        ...
        severity: bundle.severity,
        concern: bundle.concern,
        ...buildFallbackEscalationPreview(bundle),
      },
    });
  }
    ...
    analyzeInFlight.set(cacheKey, work);
        ...
        reportTitle: report.title,
	      ...
	      await persistAnalyzeArtifactsToGcp({
	        analysisLatencyMs: Date.now() - analysisStartedAt,
	        imageCount: images.length,
	        lane: liveSession.lane,
	        logChars: logs.length,
	        provider,
	        report,
	        requestId: req.requestId ?? liveSession.sessionId,
	        sessionId: liveSession.sessionId,
	      });
